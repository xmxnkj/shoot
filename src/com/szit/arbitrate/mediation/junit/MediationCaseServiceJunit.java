...
public class MediationCaseServiceJunit extends BaseApiJunitTest{
	...
	public void statMediationCaseExcelDto(){
		...
		map.put("classfy", "村调委会调解案件数");
		map.put("count", 1);
		hostNumList.add(map);
		map = Maps.newHashMap();
		map.put("classfy", "居调委会调解案件数");
		map.put("count", 2);
		hostNumList.add(map);
		map = Maps.newHashMap();
		map.put("classfy", "乡镇调委会调解案件数");
		map.put("count", 1);
		hostNumList.add(map);
		map = Maps.newHashMap();
		map.put("classfy", "街道调委会调解案件数");
		map.put("count", 3);
		hostNumList.add(map);
		map = Maps.newHashMap();
		map.put("classfy", "企事业单位调委会调解案件数");
		map.put("count", 0);
		hostNumList.add(map);
		map = Maps.newHashMap();
		map.put("classfy", "社会团体和其他组织调委会调解案件数");
		map.put("count", 1);
		hostNumList.add(map);
		...
		map = Maps.newHashMap();
		map.put("source", "依申请调解");
		map.put("count", 2);
		sourceNumList.add(map);
		map = Maps.newHashMap();
		map.put("source", "接受委托移送调解");
		map.put("count", 1);
		sourceNumList.add(map);
		map = Maps.newHashMap();
