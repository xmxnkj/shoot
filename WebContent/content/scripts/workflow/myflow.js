(function($) {
	...
	myflow.rect = function(o, r) {
		...
		$([_rect.node, _text.node, _name.node, _img.node]).bind('dblclick',function(){$(_r).trigger('nodedblclick', _this); return false;});
		...
		var dblclickHandler = function(e, src){
			//if(!myflow.config.editable)
			//	return;
			if (src.getId() == _id) {
		...
		$(_r).bind('nodedblclick', dblclickHandler);
		...
		this.toJson = function() {
			...
			for (var k in _o.props) {
				if(k=="id" && _o.props[k].value=="")
					_o.props[k].value = newGuid();
				data += k + ":{value:'"
						+ _o.props[k].value + "'},";
			}
	...
	myflow.path = function(o, r, from, to) {
		...
		$(_r).bind('dblclick', dblclickHandler);
	...
	myflow.props = function(o, r) {
		...
		var showpropsHandler = function(e, props, src) {
			...
			if(typeof(dblclickUserHandler)=="function"){
				dblclickUserHandler(props);
			}
			...
			_src = src;
			myflow.currentObject.obj = props;
			myflow.currentObject.rect = _r;
			myflow.currentObject.gsrc = src;
			
			var propUrl = "";
			if(props.propurl){
				//节点
				//$("#ifrDlg").attr("src", props.propurl);
				propUrl = props.propurl + "?type=" + props.type + "&text=" + encodeURI(encodeURI(props.text.text));
				
				for(var k in props.props){
					propUrl += "&" + props.props[k].name + "=" + encodeURI(encodeURI(props.props[k].value));
				}
				
				top.openDialog(propUrl, "属性", props.propwidth, props.propheight);
			}
			else if(props.props){
				//线
				propUrl = myflow.config.pathPropUrl + "?type=path&text=" + props.props.text.value;
				top.openDialog(propUrl, "属性", myflow.config.pathPropWidth, myflow.config.pathPropHeight);
			}else{
				//流程
			}
	...
	myflow.init = function(c, o) {
		...
		$(_r).bind('addrect', function(e, type, o) {
			...
			var newId = rect.getId();
			_states[newId] = rect;
			myflow.states[newId] = rect;
		});
		// 添加路径
		...
		if (myflow.config.editable) {
			...
			var validator = function(states, paths){
				var startNodeCount=0;
				for(var k in states){
					var rect = states[k];
					if(rect != null && rect.getType() == "start")
						startNodeCount++;
				}
				if(startNodeCount>1)
					return "流程最多只能有一个开始节点！";
				
				//var emptyInCountNodesCount=0;
				//验证task只能有一个出口
				for(var k in states){
					var rect = states[k];
					var outCount=0;
					var inCount =0;
					var rectType = rect != null ? rect.getType() : "";
					if(rect != null && (rectType == "task" || rectType=="end" || rectType=="fork")){
						for(var j in paths){
							var path = paths[j];
							
							if(path!=null && path.from().getId()==k){
								outCount++;
							}
							if(path != null && path.to().getId() == k){
								inCount++;
							}
						}
						if(outCount>0 && rectType=="end")
							return "结束节点不能有后置步骤！";
						if(outCount>1 && rectType=="task")
							return "顺序节点的下一步骤最多只能一个，步骤：[" + rect.text() + "]的下一步骤有多个";
						if(inCount==0)
							return "顺序步骤、条件步骤、结束节点必须有前置步骤，步骤：[" + rect.text() + "]没有前置步骤";
			...
			$('#myflow_save').click(function() {// 保存
						var validateResult = validator(_states, _paths);
						var data = "";
						...
						myflow.config.tools.save.onclick(data, validateResult);
						// alert(data);
					});
			// 属性框
			new myflow.props({}, _r);
