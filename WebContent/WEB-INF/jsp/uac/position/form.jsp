...
<body>
	<script>
		...
		function initForm(){
			$("#frm").form({
                url: "<s:url action="save" namespace="/uac/position" />",
                onSubmit: function () {
                ...
                success: function (data) {
                    ...
                    if(dealJsonResult(data, "职务保存成功！", function(){$("#_name").focus();})){
                    	clearForm();
                    	top.refreshContent();
                    }
			...
			$("#_name").validatebox({
				...
				missingMessage:'职务名称必填，请输入！',
				invalidMessage:'职务名称长度不超过100个汉字！',
				deltaX:-200
			});
			$("#_name").validatebox({
				validType: "remote['<s:url action="validatename" namespace="/uac/position" />?id=" + $('#_id').val() + "', 'entity.name']",
				invalidMessage:'职务名称已存在，请修改名称！',
				deltaX:-200
			});
	...
	<div class="formdiv">
			<table class="formtable">
				<tr>
					<td class="tdHeader">职务名称<label class="required">*</label>：</td>
