...
<body>
	<script>
		...
		function initForm(){
			$("#frm").form({
                url: "<s:url action="save" namespace="/uac/role" />",
                onSubmit: function () {
                ...
                success: function (data) {
                    ...
                    if(dealJsonResult(data, "角色保存成功！", function(){$("#_name").focus();})){
                    	clearForm();
                    	top.refreshContent();
                    }
			...
			$("#_name").validatebox({
				...
				missingMessage:'角色名称必填，请输入！',
				invalidMessage:'角色名称长度不超过100个汉字！',
				deltaX:-200
			});
			$("#_name").validatebox({
				validType: "remote['<s:url action="validatename" namespace="/uac/role" />?id=" + $('#_id').val() + "', 'entity.name']",
				invalidMessage:'角色名称已存在，请修改名称！',
				deltaX:-200
			});
	...
	<div class="formdiv">
			<table class="formtable">
				<tr>
					<td class="tdHeader">角色名称<label class="required">*</label>：</td>
