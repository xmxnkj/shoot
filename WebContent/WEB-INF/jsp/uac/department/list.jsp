...
<body>
	<script>
		...
		function add(){
			top.openDialog('<s:url action="edit" namespace="/uac/department" />',
					"添加部门",
					500, 500
					);
		}
		...
		function edit(){
			...
			if(row != null){
				url = '<s:url action="edit" namespace="/uac/department" />?id=' + row.id;
				top.openDialog(url,
						"修改部门信息",
						500, 500
						);
		...
		function del(){
			var row = $('#ulTree').tree('getSelected');
			if(row != null){
				$.messager.confirm('提示信息', '您确认要删除选中的部门吗？', function (data) {
                    if (data) {
                        $.ajax({
                            url: '<s:url action="delete" namespace="/uac/department" />?id=' + row.id+ "&rnd="+ Math.random(),
                            type: 'get',
                            timeout: 1000,
                            success: function (data) {
                                if(dealJsonResult(data, "部门删除成功！")){
                                	refreshData();
                                }
                            },
                            error: function () {
